...
export default function Saved() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/api/startup/${id}`, {
        headers: { Authorization: "Bearer " + localStorage.getItem("token") },
      });
  ...
  const filteredStartups = startups.filter(s => 
    s.isSaved && (s.title?.toLowerCase().includes(searchTerm.toLowerCase()) || 
      ...
      <div className="animate-fade-in">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">View all your favorited startup ideas.</p>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center flex flex-col items-center justify-center">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-1">No saved ideas found</h3>
            <p className="text-gray-500 text-sm mb-6 max-w-sm">
              {searchTerm ? "No saved ideas match your search." : "You haven't saved any startup ideas yet."}
            </p>
            {!searchTerm && (
