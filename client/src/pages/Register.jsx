...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    setError("");
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/api/user/register`, form);
      navigate("/");
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#f8fafc] relative overflow-hidden">
      ...
      <div className="bg-white/80 backdrop-blur-xl p-10 rounded-2xl shadow-xl border border-white/40 w-full max-w-md relative z-10">
        ...
        <p className="text-center mt-8 text-sm text-gray-600">
          ...
          <Link to="/" className="text-brand-600 font-semibold hover:text-brand-700 transition-colors">
            Sign in
          </Link>
        </p>
      </div>
